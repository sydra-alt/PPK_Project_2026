<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 shadow-sm">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="font-extrabold text-2xl tracking-tight text-indigo-600">
                        JARA
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('lists.index')" :active="request()->routeIs('lists.*') || request()->routeIs('task-lists.*')">
                        {{ __('Daftar Tugas (Projects)') }}
                    </x-nav-link>
                    ...
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-600 bg-white hover:text-gray-900 focus:outline-none transition ease-in-out duration-150">
                            <div class="flex items-center gap-2">
                                ...
                                @if (Auth::user()->isAdmin())
                                    <span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-indigo-100 text-indigo-700">Admin</span>
                                @endif
                            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('lists.index')" :active="request()->routeIs('lists.*') || request()->routeIs('task-lists.*')">
                {{ __('Daftar Tugas (Projects)') }}
